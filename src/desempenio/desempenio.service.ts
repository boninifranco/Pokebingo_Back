...
export class DesempenioService {
  ...
  async create(createDesempenioDto: CreateDesempenioDto): Promise<Desempenio> {
    const isRegistro = await this.registroService.findOneId(
      createDesempenioDto.jugador,
    );
    if (!isRegistro)
      throw new error(
        `No existe registro del usuario con id ${createDesempenioDto.jugador}`,
      );
    const isDesempenio = await this.findDesempenioUsuario(
      createDesempenioDto.jugador,
    );

    if (isDesempenio)
      throw new BadRequestException(
        `Ya existe desempeño para el jugador con id ${createDesempenioDto.jugador}`,
      );
    try {
      const newDesempenio: Desempenio =
        this.desempenioRepository.create(createDesempenioDto);
  ...
  async findAll(): Promise<Desempenio[]> {
    try {
      ...
      if (desempenio.length === 0)
        throw new BadRequestException(
          `No se encuentran desempeños en la base de datos`,
        );
      return desempenio;
    } catch (error) {
      throw new HttpException(
  ...
  async findOne(id: number): Promise<Desempenio> {
    try {
      ...
      if (!desempenio)
        throw new BadRequestException(`No existe el desempeño con id ${id}`);
      return desempenio;
    } catch (error) {
      throw new HttpException(
        {
  ...
  async update(
    id: number,
    updateDesempenioDto: UpdateDesempenioDto,
  ): Promise<Desempenio> {
    try {
      const criterio: FindOneOptions = { where: { id: id } };
      ...
      if (!desempenio)
        throw new BadRequestException(
          `No se encuentra el desempenio con id ${id}`,
        );
  ...
  async findDesempenioUsuario(jugadorId: number): Promise<Desempenio> {
    const desempenioUsuario = await this.desempenioRepository
      .createQueryBuilder('desempenio')
