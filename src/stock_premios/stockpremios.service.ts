...
private readonly premiosService: PremiosService){}
  async create(createStockPremiosDto: CreateStockPremiosDto): Promise<StockPremios> {
    const isPremio = await this.premiosService.findOne(Number(createStockPremiosDto.premio));
    if(!isPremio) throw new error(`No existe el premio con id ${createStockPremiosDto.premio}`)

    const isPremioStock = await this.findPremiosId(createStockPremiosDto.premio);
    if(isPremioStock)throw new BadRequestException(`Ya existe un registro de stock para el premio con id ${createStockPremiosDto.premio}`)
    try {
  ...
  async findAll(): Promise<StockPremios[]> {
    try {
      ...
      if(stockPremios.length===0) throw new BadRequestException(`No se encontraron stocksPremios en la base de datos`)
      return stockPremios;      
    } catch (error) {
      throw new HttpException({status: HttpStatus.NOT_FOUND,
        error:`Se produjo un error al enviar la petición ${error}`}, HttpStatus.NOT_FOUND)      
  ...
  async findPremiosId(premio:number):Promise<StockPremios>{
    ...
    const premioId =  await this.stockPremiosRepository.createQueryBuilder('stockPremios')
