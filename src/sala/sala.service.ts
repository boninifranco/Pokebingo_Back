...
export class SalaService {
  ...
  async create(createSalaDto: CreateSalaDto): Promise<Sala> {
    try {
      ...
      if (sala) return sala;
      else throw new Error('No se pudo crear la sala');
    } catch (error) {
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: 'Error en la creación de la sala' + error,
        },
  ...
  async update(id: number, updateSalaDto: UpdateSalaDto): Promise<Sala> {
    try {
      ...
      let sala: Sala = await this.salaRepository.findOne(criterio);
      if (!sala) throw new Error('No se encuentra la sala');
      else sala = await this.salaRepository.save(sala);
      return sala;
    } catch (error) {
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: 'Error en la actualización de la sala' + error,
        },
        HttpStatus.NOT_FOUND,
      );
    }
  ...
  async remove(id: number) {
    try {
      ...
      let sala: Sala = await this.salaRepository.findOne(criterio);
      if (!sala) throw new Error('No se encuentra la sala');
      else await this.salaRepository.delete(id);
      return `Se ha eliminado la sala: ${id}`;
    } catch (error) {
      throw new HttpException(
        {
          status: HttpStatus.NOT_FOUND,
          error: 'Error en la eliminacion de la sala ' + error,
        },
        HttpStatus.NOT_FOUND,
      );
    }
