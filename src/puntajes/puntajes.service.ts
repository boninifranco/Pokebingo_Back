...
export class PuntajesService {
    ...
    try {
      ...
      if (puntajes.length === 0)
        throw new BadRequestException(
          `No se encuentan puntajes en la base de datos`,
        );
      return puntajes;
    } catch (error) {
      throw new HttpException(
  ...
  async findOne(id: number): Promise<Puntajes> {
    try {
      ...
      if (!puntaje)
        throw new BadRequestException(`No se encuentra puntaje con id ${id}`);
      return puntaje;
    } catch (error) {
      throw new HttpException(
        {
  ...
  async update(
    id: number,
    updatePuntajesDto: UpdatePuntajesDto,
  ): Promise<Puntajes> {
    try {
      const criterio: FindOneOptions = { where: { id: id } };
